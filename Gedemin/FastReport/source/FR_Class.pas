...
// object flags
  ...
  // Для PageFooter, т.к. для него нет свойства flBandPrintChildIfInvisible
...
// helper methods
  ...
  protected
    SaveX, SaveY, SaveDX, SaveDY, SaveGX, SaveGY: Double; // Integer
  //!!!<---{gs} добавлено для считываения типа парента из потока} Julia
...
//!!!<---{gs} Julia Добавлено для считывания парента из потока}
...
{$ENDIF}
    ...
    procedure ClearVariables; {!!!} {gs} {JKL} {InitVariables}
...
const
  ...
  {!!!}
  LoadVarStreamLabel = '#$&';
  SaveVarStreamLabel = LoadVarStreamLabel + '  ';
  VarStreamLabelSize = 5;
  {!!!}
type
...
begin
  for i := 0 to frFunctionsCount - 1 do
    ...
    begin
      if Assigned(frInstalledFunctions) then
      begin
...
var 
  ...
  var
    i, cur, beg, last, oldcur, LoopPos:  Integer;
    ...
    if (DocMode = dmDesigning) and (Memo1.Count = 1) and
      ...
      for i := 0 to Memo1.Count - 1 do
        if FWrapped then
          ...
          //      - первая строка текста MEMO должна начинаться с <LEADSPACES COUNT = "N"/>
          //        где N - целое число (количество символов отступа).
          begin
            if (i = 0) then begin
              FTextIndent:= '';
              if Pos('<', TrimLeft(Memo1[i])) = 1 then begin
                sInd:= TrimLeft(Memo1[i]);
                if Pos('/>', sInd) > 0 then begin
                  sInd:= TrimLeft(Copy(sInd, 2, Pos('/>', sInd) - 2));
                  if Pos('LEADSPACES ', AnsiUpperCase(sInd)) = 1 then begin
                    sInd:= TrimLeft(Copy(sInd, 11, Length(sInd) - 10));
                    if Pos('COUNT', AnsiUpperCase(sInd)) = 1 then begin
                      sInd:= TrimLeft(Copy(sInd, 6, Length(sInd) - 5));
                      if sInd[1] = '=' then begin
                        sInd:= Trim(Copy(sInd, 2, Length(sInd) - 1));
                        if sInd[1] = '"' then begin
                            sInd:= TrimLeft(Copy(sInd, 2, Length(sInd) - 2));
                        end;
                        try
                          iInd:= StrToInt(sInd);
                          if iInd > 0 then
                            FTextIndent:= StringOfChar(#2, iInd);
                          if Canvas.TextWidth(FTextIndent) < maxwidth then begin
                            sInd:= Memo1[i];
                            Delete(sInd, 1, Pos('/>', Memo1[i]) + 1);
                            Memo1[i]:= sInd;
                          end;
                        except
                        end;
...
var
  ...
  begin
    ...
    // < nekrasov
    if Pos(#2, Str) = 1 then begin
      if Pos(FTextIndent, Str) < 1 then begin
        iCount:= 1;
        while Str[iCount + 1] = #2 do begin
          Inc(iCount);
        end;
        FTextIndent:= StringOfChar(#2, iCount);
      end;
      sTmp:= Copy(Str, 1 + Length(FTextIndent), Length(Str) - Length(FTextIndent));
      curx:= curx + Canvas.TextWidth(FTextIndent)
    end
    else begin
      sTmp:= Str;
    end;
    ExtTextOut(Canvas.Handle, curx, cury, f, @DR,
      PChar(sTmp), Length(sTmp), nil)
    //  nekrasov />
//    ExtTextOut(Canvas.Handle, curx, cury, f, @DR,
...
begin
  ...
  begin
    AddProperty('OnLastPage', [frdtBoolean], nil);
    AddProperty('OnTopAlign', [frdtBoolean], nil);
  end;
  //--->!!!
...
begin
  ...
  //!!!<---{gs} JKL PageFooterTopAlign
  else if Index = 'ONTOPALIGN' then
    Flags := (Flags and not flBandOnTopAlign) or Word(Boolean(Value)) * flBandOnTopAlign
  //--->!!!
...
begin
  ...
  //!!!<---{gs} JKL PageFooterTopAlign
    ...
    Result := (Flags and flBandOnTopAlign) <> 0
  //--->!!!
  else if Index = 'REPEATHEADER' then
...
begin
  ...
  begin
    ...
    //!!!<---{gs} JKL PageFooterTopAlign
    m := TMenuItem.Create(Popup);
    { TODO -oJKL : Считывание строки из ресурса }
    m.Caption := 'Привязать к верхней границе';//frLoadStr(SOnTopAlign);
    m.OnClick := P8Click;
    m.Checked := (Flags and flBandOnTopAlign) <> 0;
    Popup.Items.Add(m);
    //--->!!!
  end;
...
begin
  ...
  begin
    Checked := not Checked;
    if (Restrictions and frrfDontModify) = 0 then
      Flags := (Flags and not flBandOnTopAlign) + Word(Checked) * flBandOnTopAlign;
  end;
  frDesigner.AfterChange;
end;
//--->!!!
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          //!!!b
          //added by TipTop. 3/10/2003
          //Исправление ошибки, связанной с тем, что перед расчетом высоты банда
          //невызывался скрипт, в котором могла быть изменена ширина
          //кросс банда. От ширины кросс банда может зависеть высота.  
...
// new page was requested in script
  ...
  begin
    ...
    //!!!<---{gs} JKL PageFooterTopAlign
    begin
      if (Flags and flBandOnTopAlign) = 0 then
        y := Parent.CurBottomY
      else
        y := Parent.CurY;
    end;
...
//!!!<---{gs} изменено Golden Software Mikle
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //!!!b
          if (p^.Page <> nil) and (Pos('URL', t.name) = 1) then
          //!!!e
          begin
            if Pos('@', t.tag) > 0 then
              mess := t.Tag
         ...
         begin
          ...
          //!!!b
          if (Pos('URL', t.name) = 1) or (t.tag > '') then
          //!!!e
          begin
            Cursor := crHandPoint;
            mess := t.Tag;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (s <> '') and Assigned(FindObject(s)) then
...
procedure TfrDataDictionary.SaveToStream(Stream: TStream);
  ...
  begin
    ...
    begin
      ...
      {!!!}
      S := Format(SaveVarStreamLabel + '%s', [Value.Value[i]]);
      t := VarType(Value.Value[i]);
      S[VarStreamLabelSize - 1] := Char(Lo(t));
      S[VarStreamLabelSize] := Char(Hi(t));
      {!!!}
      frWriteString(Stream, S);
    end;
  end;
...
procedure TfrReport.GetVariableValue(const s: string; var v: Variant);
  ...
  begin
    Result := CurBand;
    if (Result <> nil) and (Result.DataSet = nil) then // !!!
  ...
  begin
    ...
    begin
      if s = 'VALUE' then
        ...
        v := CurTime
      else if (s = frSpecFuncs[4]) and (MasterBand <> nil) then //!!!
        v := MasterBand.Positions[psLocal]
      else if (s = frSpecFuncs[5]) and (MasterBand <> nil) then //!!!
...
//!!!<---{gs} JKL InitVariables
procedure TfrReport.ClearVariables;
var
  I, J, K: Integer;
  Bnd: TfrBand;
begin
  for I := 0 to Pages.Count - 1 do
    ...
    begin
      for J := 1 to MAXBNDS do
      begin
        Bnd := Pages[I].Bands[Bnds[J, bpData]];
        while Bnd <> nil do
        begin
          if Bnd.Typ in [btMasterData, btDetailData, btSubdetailData] then
            for K := 0 to Bnd.Values.Count - 1 do
              Bnd.Values.Value[K] := 0;
...
begin
  //!!!<---{gs} JKL InitVariables
  if FinalPass and DoublePass then
    ClearVariables;
  //--->!!!
  DoBuildReport;
  if FinalPass then
...
begin
  ...
  if Assigned(UserStorage) then begin
    //UserStorage.WriteInteger('Options\PrinterSettings', 'Copies', Copies);
    UserStorage.WriteBoolean('Options\PrinterSettings', 'Collate', Collate);
  end;
  {$ENDIF}
  //!!!e
...
begin
  ...
  begin
    ...
    begin
      ...
      {$IFDEF GEDEMIN}
        if Assigned(UserStorage) then begin
//          E1.Text:= IntToStr(UserStorage.ReadInteger('Options\PrinterSettings', 'Copies',
//            FDefaultCopies, False));
          E1.Text := IntToStr(FDefaultCopies);
          CollateCB.Checked:= UserStorage.ReadBoolean('Options\PrinterSettings', 'Collate',
            FDefaultCollate, False);
        end;
      {$ELSE}
        E1.Text := IntToStr(FDefaultCopies);
